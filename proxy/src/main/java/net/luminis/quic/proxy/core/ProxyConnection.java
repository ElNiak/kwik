...
public class ProxyConnection implements ApplicationProtocolConnection {
    ...
    public ProxyConnection(QuicConnection quicConnection) {
    ...
    void handleRequest(QuicStream quicStream) {
        ...
        try {
            ...
            // Instead of closing the connection, the stream cloud be closed (which
            // currently requires these two calls)
    ...
    /**
     * Check that file specified by argument is actually in the www dir (to prevent
     * file traversal).
     * 
     * @param fileName
     * @return
    ...
    String extractPathFromRequest(InputStream input) throws IOException {
        BufferedReader inputReader = new BufferedReader(
                new InputStreamReader(new LimitedInputStream(input, MAX_REQUEST_SIZE)));
